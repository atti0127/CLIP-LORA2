...
class HydraLinearLoRA(nn.Linear):
    """LoRA with one shared A matrix and multiple routed B matrices."""
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.scaling = lora_alpha / math.sqrt(r) if r > 0 else 1.
        self.adapters_disabled = False
        ...
        if r > 0:
            self.hydra_A = nn.Parameter(self.weight.new_empty((r, self.in_features)))
            self.hydra_B = nn.Parameter(
                self.weight.new_zeros((num_experts, self.out_features, r)))
            self.hydra_router = nn.Parameter(self.weight.new_empty((num_experts, r)))
            nn.init.kaiming_uniform_(self.hydra_A, a=math.sqrt(5))
            # Small non-identical routing weights prevent all zero-initialized B experts
            # from receiving exactly the same first updates.
            nn.init.normal_(self.hydra_router, mean=0., std=0.02)
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        hydra_input = self.dropout(x) if self.training else x
        shared_features = F.linear(hydra_input, self.hydra_A)
        router_logits = F.linear(shared_features, self.hydra_router)
        router_weights = F.softmax(
            router_logits / self.router_temperature, dim=-1)
        if self.training:
            reduce_dims = tuple(range(router_weights.dim() - 1))
            self.last_router_probs = router_weights.mean(dim=reduce_dims)
        routed_B = torch.einsum(
            '...e,eor->...or', router_weights, self.hydra_B)
        hydra_output = torch.einsum(
            '...r,...or->...o', shared_features, routed_B)
        return result + self.scaling * hydra_output
