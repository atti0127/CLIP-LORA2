...
def get_arguments():
    ...
    parser.add_argument('--accumulation_steps', default=1, type=int,
                        help='microbatches accumulated per GPU before each optimizer step')
    ...
    parser.add_argument('--adaptation', default='hydra',
                        choices=['lora', 'hydra'],
                        help='adapter architecture')
    parser.add_argument('--num_experts', default=4, type=int,
                        help='number of routed B matrices used by HydraLoRA')
    parser.add_argument('--router_temperature', default=0.1, type=float,
                        help='starting softmax temperature for HydraLoRA expert routing')
    parser.add_argument('--router_temperature_end', default=0.1, type=float,
                        help='final and inference router temperature for annealed Hydra')
    parser.add_argument('--router_temperature_schedule',
                        choices=['fixed', 'linear', 'cosine'], default='fixed',
                        help='router temperature schedule for clean Hydra training')
    parser.add_argument('--hydra_diversity_weight', default=0.0, type=float,
                        help='weight for Hydra B-expert cosine diversity loss')
    parser.add_argument('--hydra_balance_weight', default=0.0, type=float,
                        help='weight for Hydra router load-balancing loss')
